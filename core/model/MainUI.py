...
from app.insertUser.user import InsertWorkerUser as InsWorker
...
class MainUi(QFrame):
    def __init__(self):
        ...
        self.setWindowTitle("Mersad Monitoring System")
    ...
    def read_exel_file(self):
        file_name = QFileDialog.getOpenFileName(self, "Open File")
        if file_name[0]:
            self.excelAdress = file_name[0]
            self.Exel.import_pb.setEnabled(True)
            exel_file = pd.ExcelFile(file_name[0])
            df2 = exel_file.parse(0)
            self.count_row = df2.shape[0]
            self.Exel.text_edit.setText(str(df2.head(50)))

    def import_exel_file(self):
        data = pd.read_excel(self.excelAdress, index_col=0)
        self.Exel.import_pb.setEnabled(False)
        self.Exel.open_pb.setEnabled(False)
        list_f_name = data['نام']
        list_l_name = data['نام خانوادگي']
        list_user = data['كد پرسنلي']
        list_pass = data['شماره ملی']
        for i in list_f_name.index:
            InsWorker(list_f_name[i], list_l_name[i], list_user[i], "", list_pass[i])
            self.Exel.pBar.setValue(i / self.count_row * 100)

        self.Exel.import_pb.setEnabled(True)
        self.Exel.open_pb.setEnabled(True)
    ...
    def init_ui(self):
        ...
        # from MersadJsonViewer import JsonViewer as JV
        ...
        self.Exel.import_pb = self.Exel.findChild(QPushButton, "import_pb")
        self.Exel.open_pb = self.Exel.findChild(QPushButton, "open_pb")
        self.Exel.text_edit = self.Exel.findChild(QTextEdit, "excel_tx_edit")
        self.Exel.pBar = self.Exel.findChild(QProgressBar, "progressBar")

        self.Exel.import_pb.clicked.connect(self.import_exel_file)
        self.Exel.open_pb.clicked.connect(self.read_exel_file)
        self.Exel.pBar.setValue(0)
        self.Exel.import_pb.setEnabled(False)
